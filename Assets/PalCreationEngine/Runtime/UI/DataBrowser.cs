...
namespace PalCreationEngine.UI
{
    ...
    public sealed class BrowserColumn
    {
        ...
        /// <summary>
        /// True for numeric columns so sorting compares values rather than
        /// text -- otherwise "10" sorts before "9".
        /// </summary>
        public bool Numeric;
    ...
    /// <summary>
    /// A spreadsheet-style browser for the big lookup lists.
    ///
    /// The inline dropdowns work for a known ID but are hopeless for browsing:
    /// they show one column of internal IDs with no way to sort, compare, or
    /// filter by type. This shows every field as a column, with a search box per
    /// column, click-to-sort headers, and a row count that updates as filters
    /// narrow.
    ///
    /// Column 0 is always the value handed back when a row is picked, so a
    /// caller gets an ID regardless of which columns it chose to display.
    ///
    ...
    public sealed class DataBrowser : VisualElement
    {
            ...
            Action onClose = null)
        {
            ...
            Add(UiKit.Row("Search all columns", _globalSearch, "matches any field",
                labelWidth: 130, fieldWidth: 260));
        ...
        /// <summary>Column titles; clicking one sorts by it, clicking again reverses.</summary>
        ...
        /// <summary>One search box under each column heading.</summary>
        ...
        private void Refresh()
        {
            ...
            if (_sortColumn >= 0)
            {
                ...
                matches = column.Numeric
                    ? (_sortDescending
                        ? matches.OrderByDescending(r => AsNumber(r, _sortColumn))
                        : matches.OrderBy(r => AsNumber(r, _sortColumn)))
                    : (_sortDescending
            ...
            _countLabel.text = list.Count == _rows.Count
                ? $"{_rows.Count} rows"
                : $"{list.Count} of {_rows.Count} rows";
        ...
        private static double AsNumber(string[] row, int index) =>
            double.TryParse(Cell(row, index), out var value) ? value : double.MinValue;
        ...
        private VisualElement BuildRow(string[] row, int position)
        {
            ...
            // Alternating bands: 20+ columns of similar-looking IDs are hard to
            // track across without them.
            ...
            line.RegisterCallback<MouseEnterEvent>(_ =>
                line.style.backgroundColor = new Color(0.25f, 0.32f, 0.42f));
            line.RegisterCallback<MouseLeaveEvent>(_ =>
                line.style.backgroundColor = position % 2 == 1 ? UiKit.Panel : Color.clear);
