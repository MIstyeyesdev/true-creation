...
namespace RingOfNoXp.UI
{
    ...
    /// Ported from the Management Station / PalCreationEngine UiKit so all
    /// three tools look alike.
    /// </summary>
    public static class UiKit
    {
        ...
        /// <summary>
        /// Label + field on one line, hint on its own line underneath.
        /// </summary>
        ...
        /// <summary>A key/value display row for read-only reference blocks.</summary>
        public static VisualElement KV(string key, string value, Color? color = null, float labelWidth = 150, string hint = null)
        {
            var row = new VisualElement();
            row.style.flexDirection = FlexDirection.Row;
        ...
        public sealed class SearchPicker : VisualElement
        {
            ...
            /// <summary>Raised when an entry is chosen from the list.</summary>
            public event Action<string> OnSelected;

            /// <summary>
            /// Raised on every edit of the text, including clearing it. Without this
            /// a field can only ever be changed by picking from the list: deleting
            /// the text leaves the model holding the old value, and the next rebuild
            /// puts the old text straight back.
            /// </summary>
            ...
            public SearchPicker(Func<IEnumerable<(string Value, string Display)>> source, string placeholder, int maxResults = 200)
            {
                ...
                // SetValueWithoutNotify is used everywhere the code sets the text,
                // so this only ever fires for edits the user actually made.
